...
void duplicateFiles(fs::path form, fs::path to, fs::path root)
{
    for (auto &&directoryOrFile : fs::directory_iterator(form))
    {
        if (directoryOrFile.is_directory())
        {
            ...
            if (directoryName != ".git" && directoryName != ".svn" && directoryName != ".vscode" && directoryName != "logDir" && directoryName != "示意图")
            {
        ...
        {
            if (directoryOrFile.path().filename().string() != ".gitignore" && directoryOrFile.path().extension().string() != ".udf")
            {
